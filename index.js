...
const vision = Vision({ projectId: require('./config').projectId});
const token = require('./config').token

const getAdj = require('./helpers/getAdj')   
...
// @wall-e show me a cute gif
// spits out a random wall-e gif
...
//@wall-e describe this pic
controller.hears('describe this pic (.*)',['direct_message','direct_mention','mention'],function(bot,message) {
  var fileName = message.match[1];
  // console.log("file name: ", fileName)
  fileName = fileName.substr(1).slice(0, -1);
  // console.log("file name: ", fileName)  
  
  // fileName = "https://files.slack.com/files-pri/T2J67TBT5-F4TAK0802/blackpower_yellow.jpg?pub_secret=8533972599"
 
  
  var res = request.get(fileName, {        
        "Authorization":"Bearer " + token
    });

  vision.detectLabels(res['uri']['href']).then((results) => {
    const labels = results[0];
    words=labels.join(" ");

    wordpos = new WordPOS();
    wordpos.getAdjectives(words, function(response){
      var adj = ""
      adj = " "+ response[0] + " "
      if (response[0] == undefined){adj=" "}
      bot.reply(message, "This picture contains a" + adj+ labels[0])
      bot.reply(message, "full results:" + words)
    });
